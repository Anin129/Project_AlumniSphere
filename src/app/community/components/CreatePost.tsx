...
const CreatePost: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const reward = authorRole === 'Student' ? '3 stars' : '1 point';
      setShowReward(`🎉 Post created! You earned ${reward}!`);
      
      // Navigate after a short delay to show the reward
      setTimeout(() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200/60 bg-white/70 p-6 text-gray-700 space-y-5">
      ...
      {showReward && (
        <div className="rounded-md bg-green-50 text-green-700 px-3 py-2 border border-green-200 text-center font-semibold">
          {showReward}
        </div>
      )}
